...
/**
 * A Kafka listener component that processes ASN.1 encoded SSM JSON messages from a specified Kafka
...
public class RawEncodedSSMJsonRouter {
  ...
  /**
   * Constructs an instance of the RawEncodedSSMJsonRouter.
   *
  ...
  /**
   * Consumes and processes Kafka messages containing ASN.1 encoded SSM JSON data. This method
   ...
   *                       includes the raw ASN.1 encoded JSON SSM data to be processed.
   * @throws StartFlagNotFoundException If the start flag for the SSM message type is not found
   *                                    during payload processing.
   * @throws JsonProcessingException    If there's an error while processing or deserializing JSON
   *                                    data.
   */
  @KafkaListener(id = "RawEncodedSSMJsonRouter", topics = "${ode.kafka.topics.raw-encoded-json.ssm}")
