...
@SpringBootTest(
    classes = {KafkaProducerConfig.class, KafkaConsumerConfig.class, RawEncodedMAPJsonRouter.class,
        RawEncodedJsonService.class, SerializationConfig.class, TestMetricsConfig.class},
    properties = {"ode.kafka.topics.raw-encoded-json.map=topic.RawEncodedMAPJsonRouterTest"})
...
class RawEncodedMAPJsonRouterTest {
  ...
  void testListen() throws Exception {
    EmbeddedKafkaHolder.addTopics(rawEncodedMapJson);

    try (InputStream inputStream = getClass().getClassLoader().getResourceAsStream(
        "us/dot/its/jpo/ode/kafka/listeners/asn1/decoder-input-map.json")) {
      assert inputStream != null;
      var json = new String(inputStream.readAllBytes(), StandardCharsets.UTF_8);
      kafkaTemplate.send(rawEncodedMapJson, json);
    }
